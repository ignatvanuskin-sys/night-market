...
export default function Home() {
  ...
  const intentLabel = useMemo(() => { const terms = expandNaturalQuery(query); if (!query.trim() || terms.length <= query.trim().split(/\s+/).length) return "Try a mood, material, use case or object."; return `Reading intent: ${terms.slice(0, 4).join(" · ")}`; }, [query]);
  const filtered = useMemo(() => {
    ...
    if (sort === "relevance") result.sort((a, b) => Number(b.featured) - Number(a.featured));
    return result;
  ...
  return (
    <div className="nm-site">
      ...
      <FloatingCart cart={cart} cartCount={cartCount} subtotal={subtotal} discountedSubtotal={discountedSubtotal} onOpen={() => setCartOpen(true)} />
      ...
      <AnimatePresence>{cartOpen && <CartDrawer cart={cart} subtotal={subtotal} discountedSubtotal={discountedSubtotal} bundleActive={bundleActive} onClose={() => setCartOpen(false)} onChange={changeQuantity} onCheckout={() => toast.info("Demo checkout", { description: "Connect a commerce backend to enable live payments." })} />}</AnimatePresence>
