...
export default function Home() {
  ...
  const addBundleToCart = (items: Product[]) => { items.forEach((item) => setCart((current) => { const exists = current.find((line) => line.product.id === item.id); return exists ? current.map((line) => line.product.id === item.id ? { ...line, quantity: line.quantity + 1 } : line) : [...current, { product: item, quantity: 1 }]; })); setBundleActive(true); setCartPulse(true); window.setTimeout(() => setCartPulse(false), 520); setSelected(null); setCartOpen(true); toast.success("Образ добавлен со скидкой 10%", { description: "Скидка применена к полной подборке в корзине." }); };
  ...
  return (
    <div className="nm-site">
      ...
      <AnimatePresence>{selected && <ProductDrawer product={selected} onClose={() => setSelected(null)} onAdd={addToCart} onBundle={addBundleToCart} />}</AnimatePresence>
...
function ProductDrawer({ product, onClose, onAdd, onBundle }: { product: Product; onClose: () => void; onAdd: (p: Product) => void; onBundle: (items: Product[]) => void }) {
