...
import { catalogProductById } from "@/lib/catalog-data";
...
export default function Favorites() {
  ...
  return <div className="nm-site"><div className="nm-noise" aria-hidden="true" /><header className="nm-header"><Link className="nm-brand" href="/" aria-label="NIGHT MARKET home"><ProgressiveImage src={`${ASSET_BASE}FcXuAAsdOxmisWCl.png`} alt="" /><span>NIGHT<br /><em>MARKET</em></span></Link><nav className="nm-nav" aria-label="Основная навигация"><Link href="/">Каталог</Link><Link href="/lookbook">Лукбук</Link><a href="/">О проекте</a></nav><Link className="nm-text-button" href="/">← Вернуться в каталог</Link></header><main id="top" className="nm-favorites-page"><div className="nm-catalog-top"><div><p className="nm-eyebrow">Личный список</p><h1>Избранное<br /><i>после темноты.</i></h1></div><p className="nm-catalog-count">{String(items.length).padStart(2, "0")} объектов<br /><span>сохранено в браузере</span></p></div>{items.length === 0 ? <section className="nm-favorites-empty"><Heart size={26} /><h2>Здесь пока тихо.</h2><p>Сохраняйте товары сердцем, чтобы вернуться к ним позже.</p><Link className="nm-cta" href="/">В каталог <ArrowDownRight size={17} /></Link></section> : <section className="nm-product-grid">{items.map((product) => <article className="nm-favorite-card" key={product.id}><div className="nm-favorite-card-image"><ProgressiveImage src={product.image} alt={`${product.title} — изображение товара`} loading="lazy" /><button className="nm-card-favorite is-active" onClick={() => remove(product.id)} aria-label={`Удалить ${product.title} из избранного`}><Heart size={15} fill="currentColor" /></button></div><div className="nm-card-meta"><div><p className="nm-card-category">{ru(product.category)}</p><h2>{product.title}</h2><p>{product.description}</p></div><strong>{money(product.price)}</strong></div><button className="nm-add-link" onClick={() => addToCart(product)}>Добавить в корзину <Plus size={14} /></button><Link className="nm-card-detail-link" href={`/product/${catalogProductById(product.id)?.slug || product.id}`}>Подробнее <ArrowDownRight size={13} /></Link></article>)}</section>}</main><footer className="nm-footer"><Link className="nm-underlink" href="/">Вернуться в каталог <ArrowDownRight size={15} /></Link><span>© 2026 NIGHT MARKET.</span><span><ShoppingBag size={14} /> {cart.reduce((sum, line) => sum + line.quantity, 0)}</span></footer></div>;
}
